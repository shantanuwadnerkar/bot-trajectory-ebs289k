function [map, occupancyGrid] = laserScan(map, occupancyGrid, ...
...
occupancyGrid = updateOccupancyGrid(occupancyGrid, laserIJ, 5);
...
for angle = - angleSpan/2 : angleStep: angleSpan/2
    ...
    if (isinf(obstacleIJ(1)) || isinf(obstacleIJ(2)))
        ...
        if laserSpan(laserBeamCount, 2) <= rangeMax
            % change occupancyGrid
            occupancyGrid = updateOccupancyGrid(occupancyGrid, obstacleIJ, 2);
        end
